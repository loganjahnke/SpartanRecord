...
export function BreakdownTile(props: { title: string, value: number | string, total?: number, isMainStat?: boolean, isPercent?: boolean })
{
	const { title, value, total, isMainStat, isPercent } = props;
	...
	return (
		<Box sx={{ background: background, borderRadius: 3, display: "flex", flexDirection: "column", alignItems: isMainStat ? "center" : "flex-start", width: isMainStat ? "auto" : "90%", margin: 1, padding: 1 }}>
            <Typography variant="caption" sx={{ flexGrow: 1, mt: 0.5, ml: 0.5 }}>{title}</Typography>
            <Typography variant="h4" sx={{ textAlign: "left", mb: 0.5, ml: 0.5 }}>{typeof value === "number" ? (Math.round(value * 100) / 100).toLocaleString() : value}{isPercent ? "%" : ""}</Typography>
		</Box>
	);
}